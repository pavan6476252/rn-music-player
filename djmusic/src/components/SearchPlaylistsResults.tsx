...
import useSearchPlaylists, { Result } from '../hooks/useSearchPlaylist';

const SearchPlaylistsResults = (props: { query: string }) => {
    // console.log(props.query)
    const { data, loading, error, loadMore } = useSearchPlaylists(props.query);
            ...
            {data?.length ?? 0 > 0 ? (
                <>
                    <FlatList
                        ...
                        scrollEnabled={true}
                        data={data}
                        keyExtractor={(item: Result) => item.id + Math.random()}
...
export default SearchPlaylistsResults;
